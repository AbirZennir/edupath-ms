    ...
    def get_at_risk_students_list(self, db: Session, limit: int = 200) -> List[AtRiskStudent]:
        ...
        for item in scored_students:
            ...
            at_risk_list.append(AtRiskStudent(
                id=s.id_student,
                nom=f"Étudiant {s.id_student}", 
                classe=f"{s.code_module} {s.code_presentation}",
                modules=s.code_module,
                risque=int(item["risk_score"] * 100),
                derniereConnexion="2 jours", # Dummy data as per original request context or unavailable
                niveau=status_map.get(item["status"], "faible"),
